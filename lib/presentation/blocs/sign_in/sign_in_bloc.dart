...
class SignInBloc extends Bloc<SignInEvent, SignInState> {
  ...
  FutureOr<void> onCheckEmailVerificationEvent(
      CheckEmailVerificationEvent event, Emitter<SignInState> emit) async {
    ...
    result.fold(
      ...
      (r) {
        if (r) {
          emit(state.copyWith(isVerify: r, status: BlocStatus.success));
  ...
  FutureOr<void> onGetUserTypeEvent(
      GetUserTypeEvent event, Emitter<SignInState> emit) async {
    final result = await getUserTypeUseCase.call(state.userId);
    result.fold(
      (l) => emit(state.copyWith(authMessage: l.errorMessage)),
      (r) => emit(state.copyWith(userType: r)),
    );
  }

  FutureOr<void> onSetSignInStatusToDefault(
      SetSignInStatusToDefault event, Emitter<SignInState> emit) {
    emit(state.copyWith(
      signInParams: const SignInParams(email: '', password: ''),
